...
<section id="heroSection" class="container relative max-w-screen-xl pt-10 pb-24">
    <div class="flex flex-wrap items-center justify-between w-full gap-8">
        ...
        <img src="{{ asset('/assets/images/hero-image.jpg') }}" class="max-w-[584px] max-h-[400px] w-full h-full"
            alt="tickety-assets">
    </div>
</section>
<!-- Wavy line ornament -->
...
<section id="eventSection" class="container relative max-w-screen-xl py-10">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
        @foreach ($events as $event)
            <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :category="$event->name"
            :date="$event->start_time" :price="$event->start_from" :description="$event->headline" :ispopuler="$event->is_populer"
    ...
    <div class="container relative max-w-screen-xl py-10">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-[30px] relative">
            @foreach ($categories as $category)
                <x-frontend.card-category :icon="asset('/assets/svgs/ic-chart-growth.svg')" :name="$category->name" :totalEvents="$category->events_count" :route="request()->fullUrlWithQuery(['category' => $category->id])"/>
            @endforeach
        </div>
    </div>
